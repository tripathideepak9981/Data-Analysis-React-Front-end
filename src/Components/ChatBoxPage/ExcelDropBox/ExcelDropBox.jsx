...
const ExcelDropBox = () => {
  ...
  const ShowAlert = async () => {
    ...
    //           return `<p style="margin-bottom: 5px;">${sectionTitle} ${sectionContent}</p>`;
    //         })
    //         .join("")}
    //     </div>
    //   `,
    ...
    //     confirmButton:
    //       "bg-red-600 hover:bg-blue-700 text-white px-1 py-1 rounded",
    //     cancelButton:
    //       "bg-gray-500 hover:bg-gray-300 text-gray-800 px-1 py-1 rounded",
    //   },
    // });
    ...
    const { isConfirmed } = await Swal.fire({
      ...
      html: `
        <div style="
          ...
          padding: 5px; margin: 0px; border: 3px solid #ccc; border-radius: 5px; 
          background-color: #f9f9f9; font-size: 15px; font-weight: 400;">
          ${cleaningSummary
            .split("\n\n")
            .map((section) => {
              const sectionTitleMatch = section.match(/^\*\*(.*?)\*\*/);
              const sectionTitle = sectionTitleMatch
                ? `<strong style="color: #007bff; font-size: 15px;">🔹 ${sectionTitleMatch[1]}</strong><br/>`
                : "";
              const sectionContent = section.replace(/\*{1,2}/g, "").trim();
              return `<p style="margin-bottom: 0px;">${sectionTitle} ${sectionContent}</p>`;
            })
            .join("")}
        </div>
      `,
      showCancelButton: true,
      confirmButtonText: "Clean File",
      ...
      customClass: {
        ...
        confirmButton:
          "bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-xs",
        cancelButton:
          "bg-gray-500 hover:bg-gray-300 text-gray-800 px-2 py-1 rounded text-xs",
      },
    });
    ...
    try {
      ...
      if (isConfirmed) {
        ...
        if (cleanTableResponse.status === "cleaned") {
          Swal.fire({
            title: "Data Cleaned!",
            text: "Your Data has been cleaned, Successfully",
            icon: "info",
            width: "30vw",
            heightAuto: false,
            padding: "3px",
        ...
        if (cancelResponse.status !== "error") {
          ...
          Swal.fire({
            title: "Skipped",
            text: "You chose to skip for now. Default table is stored",
            icon: "info",
            width: "30vw",
            heightAuto: false,
            padding: "3px",
    ...
    <div className="w-full max-w-xl mx-auto px-4 py-4 h-[90vh] bg-white rounded-2xl shadow-2xl">
      ...
      {isLoading && (
        <div className="fixed inset-0 flex justify-center items-center z-50">
          {/* Blur Background for Inner Div Only */}
          <div className="bg-gray-400 bg-opacity-20 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-white border-opacity-25 flex flex-col items-center">
            {/* Spinner */}
            <div className="relative w-16 h-16">
              {/* Outer Ring */}
              <div className="absolute inset-0 border-4 border-transparent border-t-blue-500 border-r-blue-500 rounded-full animate-spin"></div>
              {/* Inner Ring */}
              <div className="absolute inset-1 border-4 border-transparent border-t-purple-600 border-l-purple-600 rounded-full animate-[spin_1.5s_linear_infinite_reverse]"></div>
            </div>
            {/* Loading Text */}
            <p className="text-lg font-semibold text-gray-800 mt-4">
              Storing your data, please wait...
            </p>
          </div>
        </div>
      )}
